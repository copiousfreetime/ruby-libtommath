...
module FileOperations
  ...
  def extdir?(dir)
    File.exist?("#{dir}/MANIFEST") or File.exist?("#{dir}/extconf.rb") or File.exist?("#{dir}/mkrf_conf.rb")
  end
...
class Installer
  ...
  def extconf
    if File.exist?("#{curr_srcdir()}/extconf.rb") then
      ruby "#{curr_srcdir()}/extconf.rb", *@config.config_opt
    elsif File.exist?("#{curr_srcdir()}/mkrf_conf.rb") then
      ruby "#{curr_srcdir()}/mkrf_conf.rb", *@config.config_opt
    end
  end
  ...
  def clean_dir_ext(rel)
    return unless extdir?(curr_srcdir())
    make 'clean' if (File.file?('Makefile') or File.file?('Rakefile'))
  end
  ...
  def distclean_dir_ext(rel)
    return unless extdir?(curr_srcdir())
    make 'distclean' if (File.file?('Makefile') or File.file?('Rakefile'))
  end
